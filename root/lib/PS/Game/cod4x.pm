...
#
#	$Id: cod4x.pm 514 2008-07-07 18:19:50Z lifo $
#
package PS::Game::cod4x;
...
sub event_cod4x_init {
	...
	#	if ($self->{cod4x_shutdown} > $timestamp) {
	#		$self->{gamestart} = $self->{cod4x_shutdown};
	#		$timestamp = $self->{cod4x_shutdown};
	#		$self->info("COD server restart timestamp " . date("%Y-%m-%d %H:%i:%s\n", $timestamp));
	#	}
