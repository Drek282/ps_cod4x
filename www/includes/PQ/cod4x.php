...
/**
	...
	There is an 'oldcod4x' query_type that can be used to force older cod4x queries.
...
class PQ_cod4x extends PQ_PARENT {

function __construct($conf) {
	$this->PQ_cod4x($conf);
}

function PQ_cod4x($conf) {
...
function init() {
	...
	$this->cod4x_version = 0;	// 0 = unknown version
...
// sets and returns the cod4x version to properly decode packets
function _hlver($header=NULL) {
	...
	if ($header === NULL and $this->cod4x_version) {
		return $this->cod4x_version;
	}
	if ($header === NULL) {
		$header = $this->raw;
	}
	if ($header === NULL) {		// if header is still null, we have no packet bytes to check
		$this->cod4x_version = 0;		
	} else {
		if ($this->DEBUG) print "DEBUG: Determing cod4x version from header packet:\n" . $this->hexdump(substr($header,0,16));
		$code = @substr($header, 4, 1);
		if (!$code) {
			$this->cod4x_version = 0;
		} elseif (in_array($code, array('m', 'o'))) {
			$this->cod4x_version = 1;
		} else {
			$this->cod4x_version = 2;
		}
	}
	return $this->cod4x_version;
}
...
// internal function to parse cod4xx 'info' packets
function _parse_info_cod4x() {
...
// issues an RCON command to a cod4x version 1 server (non-source)
...
// issues an RCON command to a cod4x version 2 server (source)
...
// internal function to send a non-authoritative query to a cod4x server (NOT RCON COMMANDS)
...
// internal function to send a query to the master cod4x servers (steam) for an IP list.
